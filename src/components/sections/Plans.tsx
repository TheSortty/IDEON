...
const PlanCard = ({ plan, isFeatured, index }: { plan: any; isFeatured?: boolean; index: number }) => {
  ...
  return (
    <motion.div
      ...
      className={`glass-panel p-8 flex flex-col relative overflow-hidden transition-all duration-200 ${isFeatured ? 'border-accent shadow-[0_0_40px_rgba(212,0,255,0.3)] ring-2 ring-accent/50 scale-105 z-10' : 'border-white/10 hover:border-white/30'}`}
    >
      {isFeatured && (
      ...
      <div className="flex items-center justify-start mb-2 gap-3">
        ...
        <span className={`text-xs font-bold uppercase tracking-wider px-2 py-1 rounded-md ${isFeatured ? 'bg-accent text-white' : 'bg-gray-200 dark:bg-white/10 text-gray-700 dark:text-gray-200'}`}>
          {plan.tag}
        </span>
      </div>

      <p className="mt-2 text-content-muted">{plan.description}</p>

      <div className="mt-6">
        <span className="text-2xl font-bold tracking-tight text-content">{plan.price}</span>
        <p className="mt-1 text-sm text-content-muted">{plan.priceNote}</p>
      </div>

      <div className="mt-8 flex-grow">
        {plan.includesText && (
          <p className="text-sm font-bold text-accent mb-3 uppercase tracking-wide">{plan.includesText}</p>
        )}
        <ul className="space-y-4 text-content-muted">
